...
import { addFunction } from "./functions_manager";
...
<button class="fun-save-btn save-btn">Ajouter</button>
`
		);
		...
		if (functionTBody) {
            ...
            <tr id="funlist-${funName}">
                <td><button vs-fun-id="${funName}" class="fun-open-btn open-btn">${funName}</button></td>
				<td><button vs-fun-id="${funName}" class="fun-delete-btn delete-btn">X</button></td>
			</tr>
            ` + functionTBody.innerHTML;
		}
		deletePopup();
